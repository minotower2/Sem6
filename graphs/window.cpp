...
Window::Window(QWidget *parent, double a_in, double b_in, int n_in, int k_in)
    ...
    // Коэффициенты для Чебышева (только если n <= 100)
    if (n >= 1 && n <= 100) {
        chebyshev_c.reset(new double[n]);
        make_chebyshev_coefficients(n, this->a, this->b, function, chebyshev_c.get());
    } else {
        chebyshev_c.reset(nullptr); // Не считать и не хранить, если n > 100
    }

    bessel_c.reset(new double[4 * n]); 
    make_bessel_spline_coefficients(n, x.get(), f.get(), bessel_c.get());
...
void Window::calculate_max_abs_f() {
    ...
    if (delta_x <= 0 && b > a) delta_x = (b-a) / 1000.0; 
    else if (delta_x <= 0) { 
...
void Window::calculate_min_max() {
    ...
    for (x_0_val = show_a; x_0_val <= show_b + std::numeric_limits<double>::epsilon(); x_0_val += delta_x) {
        ...
        // Чебышев (только если n <= 100)
        if (mode == 1 || mode == 3 || mode == 4 || mode == 6) { 
            if (n >= 1 && n <= 100 && chebyshev_c && !std::isnan(chebyshev_c[0])) {
        ...
        if (mode == 2 || mode == 3 || mode == 5 || mode == 6) { 
            if (n >= 2 && bessel_c && !std::isnan(bessel_c[0])) { 
                y_0_val = (mode == 2 || mode == 3) ? calculate_bessel_spline_approximation(x_0_val, n, x.get(), bessel_c.get())
                                               : calculate_bessel_spline_discrepancy(x_0_val, function, n, x.get(), bessel_c.get());
                if (!std::isnan(y_0_val)) {
                    if (!first_val_set || (is_discrepancy_mode && mode > 3) ) { min_y = max_y = y_0_val; first_val_set = true; }
                    else { min_y = std::min(min_y, y_0_val); max_y = std::max(max_y, y_0_val); }
                }
...
void Window::increase_points() {
    ...
    if (n >= 1 && n <= 100) {
        ...
        chebyshev_c.reset(nullptr); // Если n стало > 100, сбрасываем
    }
...
void Window::decrease_points() {
    ...
    if (n >= 1 && n <= 100) {
        ...
        chebyshev_c.reset(nullptr); // Если n стало > 100 (маловероятно при уменьшении, но для полноты)
    }
...
void Window::paintEvent(QPaintEvent* event_paint) {
    ...
    QPen pen_red(QColor("red"), 3);         
    QPen pen_magenta(QColor("magenta"), 3);  
    QPen pen_cyan(QColor("darkCyan"), 3);     
    ...
    if (mode == 2 || mode == 3) { // Аппроксимация
        if (n >= 2 && bessel_c && !std::isnan(bessel_c[0]) ) { // Бессель требует n>=2
            painter.setPen(pen_cyan);
            drawGraph(painter, std::bind(calculate_bessel_spline_approximation, std::placeholders::_1, n, x.get(), bessel_c.get()));
        }
    } else if (mode == 5 || mode == 6) { // Невязка
         if (n >= 2 && bessel_c && !std::isnan(bessel_c[0]) ) {
            painter.setPen(pen_cyan);
            drawGraph(painter, std::bind(calculate_bessel_spline_discrepancy, std::placeholders::_1, function, n, x.get(), bessel_c.get()));
        }
    }
